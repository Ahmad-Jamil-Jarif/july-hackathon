...
export function MemorialHeatmapMap({
  labels,
  mapHeightClassName = "h-[min(65vh,560px)]",
  showHeader = false,
}: MemorialHeatmapMapProps) {
    ...
    async (map: maplibregl.Map) => {
      ...
      try {
        ...
        const res = await fetch(
          `${apiBase}/api/v1/memorial/geojson?${params.toString()}`,
          { signal: ac.signal },
        )
        if (!res.ok)
        ...
        const src = map.getSource("memorial-heat") as
        ...
        setStatus("error")
        setErrorMessage(
          e instanceof Error ? e.message : labels.error,
        )
      }
    },
    [labels.error],
  ...
  useEffect(() => {
    ...
    const map = new maplibregl.Map({
      container: el,
      style: (envStyle || DEFAULT_RASTER_STYLE) as
        | string
  ...
  const mapBlock = (
    <div className="relative overflow-hidden rounded-xl border border-border shadow-sm">
      <div
        ref={containerRef}
        className={`w-full min-h-[280px] ${mapHeightClassName}`}
        role="application"
      />
